...
public class OtherFeeServiceImpl extends BaseServiceImpl<OtherFee, Long> implements OtherFeeService {
	...
	public OtherFee addOtherFee(OtherFeeDto otherFeeDto, UserTicket userTicket) {
		...
		// 生成 其他收费 code
		String otherFeeCode = uidRpcResolver
				.bizNumber(userTicket.getFirmCode() + "_" + BizNumberTypeEnum.OTHER_FEE.getCode());
		...
		Long mchId = mchAndDistrictService.getMchIdByDistrictId(otherFeeDto.getFirstDistrictId(),
				otherFeeParam.getSecondDistrictId());
		if (mchId == null) {
			// 沈阳市场的收费性才会绑定组织
			List<DepartmentChargeItem> ChargeItemList = departmentChargeItemService
					.listChargeByChargeItemId(otherFeeDto.getChargeItemId());
	...
	public OtherFee submit(Long id, UserTicket userTicket) {
		...
		paymentOrder.setDistrictId(otherFeeInfo.getSecondDistrictId() == null ? otherFeeInfo.getFirstDistrictId()
				: otherFeeInfo.getSecondDistrictId());
	...
	public OtherFee withdraw(Long id, UserTicket userTicket) {
		...
		PaymentOrder paymentOrder = this.findPaymentOrder(userTicket, otherFeeInfo.getId(), otherFeeInfo.getCode(),
				PaymentOrderStateEnum.NOT_PAID.getCode());
	...
	public OtherFee refund(OtherFeeRefundOrderDto refundOrderDto, UserTicket userTicket) {
		...
		// 查询已缴费的缴费单（方法抽取）
		this.findPaymentOrder(userTicket, otherFeeInfo.getId(), otherFeeInfo.getCode(),
				PaymentOrderStateEnum.PAID.getCode());
		...
		// 构建退款单以及新增
		refundOrderDto.setCode(uidRpcResolver.bizNumber(
				userTicket.getFirmCode() + "_" + BizTypeEnum.getBizTypeEnum(BizTypeEnum.OTHER_FEE.getCode()).getEnName()
						+ "_" + BizNumberTypeEnum.REFUND_ORDER.getCode()));
		refundOrderDto.setBizType(BizTypeEnum.OTHER_FEE.getCode());
		refundOrderDto.setDistrictId(otherFeeInfo.getSecondDistrictId() == null ? otherFeeInfo.getFirstDistrictId()
				: otherFeeInfo.getSecondDistrictId());
...
//        }
	...
	public OtherFee settlementDealHandler(SettleOrder settleOrder) {
		...
		if (!paymentOrderPO.getState().equals(PaymentOrderStateEnum.NOT_PAID.getCode())) {
			logger.info("缴费单状态已变更！状态为："
					+ PaymentOrderStateEnum.getPaymentOrderStateEnum(paymentOrderPO.getState()).getName());
	...
	public PrintDataDto<OtherFeePrintDto> receiptRefundPrintData(String orderCode, String reprint) {
		...
		if (CollectionUtil.isEmpty(refundOrders)) {
			...
			if (SettleWayEnum.CARD.getCode() == order.getWay()) {
				// 园区卡支付
				settleDetails = settleDetails + "退款方式：" + SettleWayEnum.getNameByCode(order.getWay()) + "     园区卡号："
						+ order.getTradeCardNo();
			} else if (SettleWayEnum.CASH.getCode() == order.getWay()) {
				// 现金
				settleDetails = settleDetails + "退款方式：" + SettleWayEnum.getNameByCode(order.getWay());
