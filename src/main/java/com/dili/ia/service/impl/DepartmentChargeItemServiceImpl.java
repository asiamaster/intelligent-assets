 ...
 */
@Service
public class DepartmentChargeItemServiceImpl extends BaseServiceImpl<DepartmentChargeItem, Long>
		implements DepartmentChargeItemService {
	...
	public void batchUpdateChargeItems(UserTicket userTicket) {
		...
		List<BusinessChargeItemDto> chargeItemDtos = businessChargeItemService.queryBusinessChargeItemConfig(
				userTicket.getFirmId(), BizTypeEnum.OTHER_FEE.getCode(), YesOrNoEnum.YES.getCode());
	 ...
	 */
	@Override
	public void addDepartmentChargeItems(DepartmentChargeItemDto departmentChargeItemDto, UserTicket userTicket)
			throws BusinessException {
		// 根据收费项查询关联的部门集合
		List<DepartmentChargeItem> itemList = this.getActualDao()
				.listChargeByChargeItemId(departmentChargeItemDto.getChargeItemId());
		if (itemList == null || itemList.size() == 0) {
	...
	public String listNoParam() {
		...
		List<DepartmentChargeItem> chargeItemListByGroup = this.getActualDao()
				.listGroupByChargeItemId(userTicket.getFirmId());
			...
			for (Object Item : ItemIdSet) {
				...
				for (DepartmentChargeItem itemInfo : departmentChargeItemList) {
					if (itemInfo.getChargeItemId().equals(Item)
							&& StringUtils.isNotEmpty(itemInfo.getDepartmentName())) {
				...
				if (StringUtils.isNotEmpty(departmentNameStrBuff.toString())) {
					departmentName = departmentNameStrBuff.toString().substring(0,
							departmentNameStrBuff.toString().length() - 1);
				}
		...
		long total = chargeItemListByGroup instanceof Page ? ((Page) chargeItemListByGroup).getTotal()
				: (long) chargeItemListByGroup.size();
