<script>
    ...
    $(function () {
        ...
        _grid.bootstrapTable('refreshOptions', {url: '${contextPath}/otherFee/listPage.action', pageSize: parseInt(size)});
    });
    ...
    function openViewHandler(id) {
        ...
        dia = bs4pop.dialog({
            title: '其他收费详情',
            content: '/otherFee/view.action?id='+id,
            isIframe : true,
            closeBtn: true,
            backdrop : 'static',
            width: '800px',
            ...
            bs4pop.confirm('提交后该信息不可更改，并且可进行缴费，确认提交？', {}, function (sure) {
                if(sure){
                    bui.util.debounce(function () {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "${contextPath}/otherFee/submit.action",
        ...
        if(isSelectRow()){
            bs4pop.confirm('撤回之后该业务单可继续修改，但不能交费，如需继续交费可以再次提交。确定撤回？', {}, function (sure) {
                if(sure){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${contextPath}/otherFee/withdraw.action",
                        data: {id: selectedRow.id},
                        processData:true,
                        dataType: "json",
                        success : function(ret) {
        ...
        if(isSelectRow()){
            bs4pop.confirm('确定取消该业务单？', {}, function (sure) {
                if(sure){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${contextPath}/otherFee/cancel.action",
                        data: {id: selectedRow.id},
                        processData:true,
                        dataType: "json",
                        success : function(ret) {
    ...
    _grid.on('check.bs.table', function (e, row, $element) {
        ...
        if (state == ${@com.dili.ia.glossary.otherFeeStateEnum.CREATED.getCode()}) {
        ...
        } else if (state == ${@com.dili.ia.glossary.otherFeeStateEnum.CANCELD.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
        } else if (state == ${@com.dili.ia.glossary.otherFeeStateEnum.SUBMITTED.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
            $('#btn_withdraw').attr('disabled', false);
        } else if (state == ${@com.dili.ia.glossary.otherFeeStateEnum.PAID.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
        }
