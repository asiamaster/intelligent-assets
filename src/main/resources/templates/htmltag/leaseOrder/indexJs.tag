<script>
    ...
    _grid.on('check.bs.table', function (e, row, $element) {
        ...
        if (state == ${@com.dili.ia.glossary.LeaseOrderStateEnum.CREATED.getCode()}) {
            ...
            if(row.payState == ${@com.dili.ia.glossary.PayStateEnum.NOT_PAID.getCode()}){
                $('#btn_submit').attr('disabled', false);
            }
            //未交清且未发起过退款申请
            if (row.payState == ${@com.dili.ia.glossary.PayStateEnum.NOT_PAID.getCode()} && row.refundState == ${@com.dili.ia.glossary.RefundStateEnum.WAIT_APPLY.getCode()}) {
