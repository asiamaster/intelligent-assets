<script>
    ...
    $(function () {
        ...
        _grid.bootstrapTable('refreshOptions', {url: '${contextPath}/fee/message/listPage.action', pageSize: parseInt(size)});
    });
    ...
    function openInsertHandler() {
        dia = bs4pop.dialog({
            title: '新增信息费',//对话框title
            content: '${contextPath}/fee/message/add.html', //对话框内容，可以是 string、element，$object
            width: '80%',//宽度
            height: '95%',//高度
    ...
    function openUpdateHandler(type) {
        ...
        dia = bs4pop.dialog({
            ...
            content: '${contextPath}/fee/message/update.html?type='+type+'&code='+rows[0].code, //对话框内容，可以是 string、element，$object
            width: '80%',//宽度
            height: '95%',//高度
            isIframe: true,//默认是页面层，非iframe
            btns: [{label: '取消',className: 'btn-secondary',onClick(e, $iframe){
    ...
    function openViewHandler(code) {
        ...
        dia = bs4pop.dialog({
            title: '马甲办理详情',
            content: '/fee/message/view.html?code='+code,
            isIframe : true,
            closeBtn: true,
            backdrop : 'static',
            width: '80%',
            ...
            bs4pop.confirm('提交后该信息不可更改，并且可进行缴费，确认提交？', {}, function (sure) {
                if(sure){
                    bui.util.debounce(function () {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "${contextPath}/fee/message/submit.action",
        ...
        if(isSelectRow()){
            bs4pop.confirm('撤回之后该业务单可继续修改，但不能交费，如需继续交费可以再次提交。确定撤回？', {}, function (sure) {
                if(sure){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${contextPath}/fee/message/withdraw.action",
                        data: {code: selectedRow.code},
                        processData:true,
                        dataType: "json",
                        success : function(ret) {
        ...
        if(isSelectRow()){
            bs4pop.confirm('确定取消该业务单？', {}, function (sure) {
                if(sure){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${contextPath}/fee/message/cancel.action",
                        data: {code: selectedRow.code},
                        processData:true,
                        dataType: "json",
                        success : function(ret) {
    ...
    function openRefundHandler(){
    	if(isSelectRow()){
    		...
    		dia = bs4pop.dialog({
                title: '退款申请',//对话框title
                content: '${contextPath}/fee/message/refundApply.html?code='+selectedRow.code, //对话框内容，可以是 string、element，$object
                width: '80%',//宽度
                height: '95%',//高度
                isIframe: true,//默认是页面层，非iframe
                btns: [{label: '取消',className: 'btn-secondary',onClick(e, $iframe){
    ...
    _grid.on('check.bs.table', function (e, row, $element) {
        ...
        } else if (state == ${@com.dili.ia.glossary.MessageFeeStateEnum.CANCELLED.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
        } else if (state == ${@com.dili.ia.glossary.MessageFeeStateEnum.SUBMITTED_PAY.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
            $('#btn_withdraw').attr('disabled', false);
        } else if (state == ${@com.dili.ia.glossary.MessageFeeStateEnum.NOT_STARTED.getCode()}
        || state == ${@com.dili.ia.glossary.MessageFeeStateEnum.IN_EFFECTIVE.getCode()}) {
            $('#toolbar button').attr('disabled', true);
            $('#btn_view').attr('disabled', false);
            $('#btn_add').attr('disabled', false);
